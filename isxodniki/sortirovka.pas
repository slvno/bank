unit sortirovka;  //Название модуля
...
var
  ...
  varianti,va : array of integer;  //Переменная динамический массив возможные
                               //варианты сроков хранения. В работе
                               //3,6,9,12

  procedure sort( max:integer; osn_:TSum_Srokov); //Объявление процедуры
  //Поиск всех вариантов сроков хранения, которыми можно представить исходный
  //срок хранения (kol_mes).

  function sort1( osn_:TSum_Srokov):TSum_Srokov;  //Объявление функции
  //упорядочивония значений комбинаций сроков хранения. Из 1 - 2  3 - 1  1 - 6
  //получаем 1 - 8  3 - 1
  // 8 вложений по 1 месяцу + 1 вложение по 3 м.

  function sort2( osn_:TSroki):TSroki;  //Объявление функции упорядочивания,
  //удаление повторяющихся одинаковых комбинаций сроков вложений, т.е Например,
  //из трех одинаковых комбинаций сроков вложений(1 - 2  5 - 2) получаем
  //одну комбинацию срока вложения (1 - 2  5 - 2)

  function sort3( osn_:TSroki):TSroki;  //Объявление функции упорядочивания,
  //удаление отсутствующих комбинаций сроков вложений, т.е Например, в
  // комбинации (1 - 2  5 - 2) присутствует срок хранения 5, который может
  //отсутствовать в допустимых сроках хранения, заданных динамическим массивом
  //varianti
 ...
 //Поиск всех вариантов сроков хранения которыми можно представить исходный срок
  //хранения (kol_mes).
  procedure Polucit_Variants(kol_mes:integer);
...
//       |          |
  ...
  //одинаковых комбинаций сроков вложений, т.е Например, из трех (1 - 2  5 - 2)
  //получаем одну комбинацию (1 - 2  5 - 2)

  Sroki:=sort3(Sroki);  //удаление отсутствующих комбинаций сроков вложений, т.е
  // Например, в комбинации (1 - 2  5 - 2) присутствует срок хранения 5, который
  // может отсутствовать в допустимых сроках хранения, заданных динамическим
  //массивом varianti
...
//используемых в работе, - менее 5 минут на компьютере "b".

end;

//Определение функции проверки. ф-я определяет имеется ли это кол-во месяцев
...
begin
  ...
  //хранения varianti, если определен то выполняется составной оператор
    begin
...
//Определение функции упорядочивания,
//удаление отсутствующих комбинаций сроков вложений, т.е Например, в
//комбинации (1 - 2  5 - 2) присутствует срок хранения 5, который может
//отсутствовать в допустимых сроках хранения, заданных динамическим массивом
//varianti
function sort3( osn_:TSroki):TSroki;
label 1;         //Объявление меток для переходов
var J,              //Счетчики
    I,              //
    l               //
    :integer;
...
begin
  ...
  //различных комбинация возможных вариантов сроков хранения до последнего, его
  //номер определяется length(osn_)-1
    begin

      if assigned(osn[i]) then  //Проверяем существование исходого  массива
      //различных комбинация возможных вариантов сроков хранения. Проверка
      //делается путем сравнения с нулевым указателем nil, результат true если
      //указатель не нулевой
        for j:= 0 to length(osn_[i])-1 do //В цыкле от первого элемента исходого
        //массива комбинации возможных вариантов сроков хранения до
        //последнего, его номер определяется length(osn_[i])-1
          begin
            //Ищем срок хранения osn[i][j].chislo в массиве сроков хранения
            //varianti. Если он не найден, то выполняется составной оператор
            if not est_znachenie(osn[i][j].chislo) then  /// osn[i] - массив
            //описывающий одну комбинацию, дающую в сумме срок хранения в
            //отчетном периоде (- 12 мес). Например
            //osn[j][l].chislo - число месяцев, osn[j][l].kol - количество
            //повторений данного месяца, - один l-ый элемент массива osn[j]
              begin
                //Если найден отсутствующий срок хранения,
                //комбинация сроков хранения очищается. Для
...
1:
  ...
  //Итогом работы этой процедуры будет массив, в котором нет отсутствующих
  //сроков хранения в массиве varianti
...
//отчетном периоде (в работе - 12 мес)
          ...
          //выполняется составной оператор
            begin
                ...
                //указателем nil, результат true если указатель не нулевой
                  ...
                  //описывающий одну комбинацию, дающую в сумме срок хранения в
                  //отчетном периоде (в работе - 12 мес). Например
                  //osn[j][l].chislo - число месяцев, osn[j][l].kol - количество
                 //повторений данного месяца, - один l-ый элемент массива osn[j]
                    begin
                      //Размер массива
          ...
          if b=1 then
            begin
             ...
             if est_znachenie(b) then  //Проверяем, что 1 месяц входит в число
              ...
              //массиве Sroki.
               begin
                 dlina1:=length(Sroki);  //Определяем размер результирующего
                 //массива комбинаций вариантов сроков хранения
                              ...
                              //Если входит, то
                                 ...
                                 if est_znachenie(p) then
                                   ...
                                   begin
                                     ...
                                     //массива
                                     setlength(Sroki,dlina1+1);//увеличиваем
                                     //размер массива на 1 элемент

                                     Sroki[dlina1]:=test; //Добавляем 1 элемент,
                                     //комбинация возможных вариантов сроков
                                     //хранения
                                   end;
